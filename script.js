...
function interest(amountBorrowed, rate) {
  ...
  while (
    ...
    new Date(
      ...
      currentDate.getDate() + futureDays
    )
  ) {
    // Exit loop when incrementDate surpasses current date
...
function updateTable(array) {
  ...
  for (let i = 0; i < selectedObjects.length; i++) {
    ...
    const dailyInterestCell = document.createElement("td");
    ...
    row.appendChild(dailyInterestCell);

    // Use setTimeout to add a delay before appending each row
    setTimeout(() => {
      tableData.appendChild(row);
      row.style.animation = "fadeIn 0.5s ease-in-out";
    }, i * 200); // 500ms delay for each row
  }
}